...
const ServerError = () => {
  ...
  const errorCode = location.pathname.split('/').pop() || '500';
  
  // Error configurations for different status codes
  const errorConfigs = {
    '500': {
      title: 'Internal Server Error',
      subtitle: 'The mystical servers are experiencing turbulence',
      description: 'Something unexpected happened on our end. The digital realm is in chaos, but our wizards are working to restore order. This might be a temporary disturbance in the force.',
      icon: '🔧',
    ...
    '501': {
      ...
      subtitle: 'This mystical feature is not yet available',
      description: 'The server does not support the requested functionality. This feature may be under development or not yet implemented in our mystical archives.',
      icon: '🔮',
    ...
    '502': {
      title: 'Bad Gateway',
      subtitle: 'The gateway between realms is malfunctioning',
      description: 'The server received an invalid response from an upstream server. The mystical gateway is experiencing issues, but our guardians are working to restore the connection.',
      icon: '🌐',
    ...
    '503': {
      ...
      subtitle: 'The mystical service is temporarily unavailable',
      description: 'The service is temporarily unavailable due to maintenance or high load. Our mystical servers are taking a brief rest, but they will return stronger than ever.',
      icon: '🛠️',
    ...
    '504': {
      ...
      subtitle: 'The gateway took too long to respond',
      description: 'The gateway timed out while waiting for a response from an upstream server. The mystical connection is taking longer than expected, but patience is a virtue.',
      icon: '⏱️',
      color: 'from-orange-400 to-red-400',
      bgColor: 'via-orange-900/20'
    },
    '505': {
      ...
      subtitle: 'This protocol version is not supported',
      description: 'The server does not support the HTTP protocol version used in the request. Please use a different client or contact our mystical support team.',
      icon: '📜',
      color: 'from-purple-400 to-blue-400',
      bgColor: 'via-purple-900/20'
    }
  ...
  const config = errorConfigs[errorCode] || errorConfigs['500'];

  // Load error details from session storage
  useEffect(() => {
    const storedError = sessionStorage.getItem('lastServerError');
    if (storedError) {
      try {
    ...
    <div className={`min-h-screen bg-gradient-to-br from-slate-900 ${config.bgColor} to-slate-900 flex items-center justify-center relative overflow-hidden`}>
      ...
      <div className="text-center text-white relative z-10 max-w-2xl mx-auto px-6">
        ...
        <div className="mt-8 pt-8 border-t border-slate-700">
          <p className="text-slate-400 mb-4">If the problem persists:</p>
        ...
        <div className="mt-6 text-xs text-slate-500">
          <p>Server Status: {isRetrying ? 'Attempting Recovery' : 'Error Detected'}</p>
      ...
      <ContactSupportModal
        ...
        errorDetails={errorDetails ? 
          ...
          `500 Server Error - ${new Date().toISOString()} - URL: ${window.location.href}`
        }
      />
